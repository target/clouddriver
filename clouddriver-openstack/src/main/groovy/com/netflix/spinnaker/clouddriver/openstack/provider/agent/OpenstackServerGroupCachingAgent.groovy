...
class OpenstackServerGroupCachingAgent extends AbstractOpenstackCachingAgent {
  ...
  /**
   * Builds ASG map from stack definition.
   * @param stack
   * @return
   */
  Map<String, Object> buildASG(Stack stack) {
    Map<String, Object> result = Maps.newHashMap()

    if (stack) {
      result.put('createdTime', stack.creationTime
        ? ZonedDateTime.parse(stack.creationTime, DateTimeFormatter.ISO_OFFSET_DATE_TIME).toInstant().toEpochMilli() : ZonedDateTime.now().toInstant().toEpochMilli())
  ...
  Map<String, Object> buildImage(ProviderCache providerCache, String image) {
    ...
    CacheData cacheData = providerCache.get(IMAGES.ns, Keys.getImageKey(image, accountName))
    if (cacheData) {
