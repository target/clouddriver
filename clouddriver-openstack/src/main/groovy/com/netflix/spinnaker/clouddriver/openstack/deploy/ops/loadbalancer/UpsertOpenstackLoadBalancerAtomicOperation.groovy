...
class UpsertOpenstackLoadBalancerAtomicOperation implements AtomicOperation<Map>, TaskStatusAware, LoadBalancerResolver, StackPoolMemberAware {
  ...
  BlockingStatusChecker blockingStatusChecker
  ...
  protected LoadBalancerV2 createLoadBalancer(String region, String name, String subnetId) {
    ...
    LoadBalancerV2 result = BlockingStatusChecker.from { LoadBalancerV2 loadBalancer ->
      LbProvisioningStatus.ACTIVE == clientProvider.getLoadBalancer(region, loadBalancer.id)?.provisioningStatus
    }.execute { clientProvider.createLoadBalancer(region, name, description, subnetId) }
  ...
  protected void removeListenersAndPools(String region, String loadBalancerId, Collection<ListenerV2> listeners) {
    BlockingStatusChecker pollingClientAdaptor = getBlockingStatusChecker(region, loadBalancerId)
  ...
  protected void removeHealthMonitor(String region, String loadBalancerId, String id) {
    ...
    getBlockingStatusChecker(region, loadBalancerId).execute { clientProvider.deleteMonitor(region, id) }
    task.updateStatus UPSERT_LOADBALANCER_PHASE, "Removed existing monitor ${id} in ${region}."
  }

  /**
   * Creates and returns a polling client adaptor singleton.
   * @param region
   * @param loadBalancerId
   * @return
     */
  protected BlockingStatusChecker getBlockingStatusChecker(String region, String loadBalancerId) {
    if (blockingStatusChecker == null) {
      blockingStatusChecker = BlockingStatusChecker.from {
        LbProvisioningStatus.ACTIVE == clientProvider.getLoadBalancer(region, loadBalancerId)?.provisioningStatus
      }
    }

    blockingStatusChecker
  }
