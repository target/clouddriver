...
class DeleteOpenstackLoadBalancerAtomicOperation implements AtomicOperation<Void> {
  ...
  /*
  * curl -X POST -H "Content-Type: application/json" -d  '[ {  "deleteLoadBalancerPool": { "loadBalancerId": "6adc02a8-7b01-4f90-9e6f-9a4c3411e7ad", "region": "default", "account":  "test" } } ]' localhost:7002/openstack/ops
  */

  @Override
  Void operate(List priorOutputs) {
    String region = description.region
    String loadBalancerId = description.loadBalancerId
    ...
    try {
      ...
      LbPool lbPool = clientProvider.getLoadBalancerPool(loadBalancerId)
