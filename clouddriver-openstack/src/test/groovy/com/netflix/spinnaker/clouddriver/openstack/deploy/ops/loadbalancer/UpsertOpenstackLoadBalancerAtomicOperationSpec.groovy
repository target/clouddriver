...
class UpsertOpenstackLoadBalancerAtomicOperationSpec extends Specification {
  ...
  @Shared
  BlockingStatusChecker blockingClientAdapter = BlockingStatusChecker.from{ true }
  ...
  def "remove listeners and pools"() {
    ...
    then:
    1 * operation.getBlockingStatusChecker(region, loadBalancerId) >> blockingClientAdapter
  ...
  def "get polling client adaptor"() {
    given:
    String loadBalancerId = UUID.randomUUID()
    ...
    when:
    BlockingStatusChecker result = operation.getBlockingStatusChecker(region, loadBalancerId)

    then:
    result == operation.getBlockingStatusChecker(region, loadBalancerId)
