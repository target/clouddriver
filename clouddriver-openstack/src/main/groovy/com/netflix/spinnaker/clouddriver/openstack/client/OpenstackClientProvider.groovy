...
abstract class OpenstackClientProvider {
  ...
  int getInternalLoadBalancerPort(LbPool pool) {
    int minPort = 1
    int maxPort = (1 << 16) - 1
    String descriptionPattern = ".*internal_port=([0-9]+).*"

    Pattern pattern = Pattern.compile(descriptionPattern)
    Matcher matcher = pattern.matcher(pool.description)
    int internalPort = 0
    if (matcher.matches()) {
   ...
   * @return
   */
  String getIpForInstance(String instanceId) {
    Server server = getServerInstance(instanceId)
    /* TODO
   ...
   * @param weight
   */
  Member addMemberToLoadBalancerPool(String ip, String lbPoolId, int internalPort, int weight) {
    //TODO use handleRequest once that is refactored
    try {
      client.networking().loadbalancers().member().create(
   ...
   * @return
   */
  ActionResponse removeMemberFromLoadBalancerPool(String memberId) {
    //TODO use handleRequest once that is refactored
    try {
      client.networking().loadbalancers().member().delete(memberId)
    } catch (Exception e) {
   ...
   * @param lbPool
   */
  String getMemberIdForInstance(String ip, LbPool lbPool) {
    //TODO use handleRequest once that is refactored
    String memberId = ""
    try {
      memberId = client.networking().loadbalancers().member().list()?.find { m -> m.address == ip }?.id
    } catch (Exception e) {
   ...
   * @return
   */
  Server getServerInstance(String instanceId) {
    //TODO use handleRequest once that is refactored
