  ...
  Void operate(List priorOutputs) {
    ...
    description.loadBalancerIds.each { lb ->
      description.instanceIds.each { id ->
        ...
        LbPool pool = provider.getLoadBalancerPool(lb)
        ...
        if (action) {
          ...
          provider.addMemberToLoadBalancerPool(ip, lb, internalPort, description.weight)
        } else {
          task.updateStatus basePhase, "Getting member id for server instance $id and ip $ip on load balancer $pool.name"
          String memberId = provider.getMemberIdForInstance(ip, pool)
