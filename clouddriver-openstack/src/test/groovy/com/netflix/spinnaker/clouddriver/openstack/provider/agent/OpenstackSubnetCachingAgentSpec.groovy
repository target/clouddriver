...
class OpenstackSubnetCachingAgentSpec extends Specification {
  ...
  void "test load data"() {
    ...
    1 * cachingAgent.getAccountName() >> account
    1 * credentials.provider >> provider
    1 * provider.listSubnets(region) >> [subnet]
    1 * subnet.id >> subnetId
    1 * objectMapper.convertValue(subnet, OpenstackInfrastructureProvider.ATTRIBUTES) >> subnetAttributes
