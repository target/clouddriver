...
class AbstractRegistrationOpenstackInstancesAtomicOperationUnitSpec extends Specification {
  ...
  def setup() {
    ...
    description = new OpenstackInstancesRegistrationDescription(loadBalancerIds: LB_IDS, instanceIds: INSTANCE_IDS, weight: 1, account: ACCOUNT_NAME, credentials: credentials)
    pool = Mock(LbPool)
  }
  ...
  def "should perform registration oepration with load balancers"() {
    ...
    LB_IDS.each { lbid ->
      INSTANCE_IDS.each { id ->
        1 * provider.getLoadBalancerPool(lbid) >> pool
        1 * provider.getIpForInstance(id) >> ip
        if (method == 'registerInstancesWithLoadBalancer') {
          1 * provider.getInternalLoadBalancerPort(pool) >> port
          1 * provider.addMemberToLoadBalancerPool(ip, lbid, port, description.weight)
        } else {
          1 * provider.getMemberIdForInstance(ip, pool) >> memberId
          1 * provider.removeMemberFromLoadBalancerPool(memberId)
        }
      }
    }
    noExceptionThrown()
  ...
  def "should throw exception when load balancer not found"() {
    ...
    LB_IDS.each { lbid ->
      INSTANCE_IDS.each { id ->
        provider.getLoadBalancerPool(lbid) >> { throw new OpenstackProviderException("foobar") }
      }
    }
  ...
  def "should throw exception when server has no IP"() {
    ...
    LB_IDS.each { lbid ->
      INSTANCE_IDS.each { id ->
        _ * provider.getLoadBalancerPool(lbid) >> pool
        provider.getIpForInstance(id) >> { throw new OpenstackProviderException("foobar") }
      }
    }
    Exception ex = thrown(OpenstackProviderException)
    ex.message == "foobar"
  ...
  def "should throw exception when internal port is not found"() {
    ...
    LB_IDS.each { lbid ->
      INSTANCE_IDS.each { id ->
        _ * provider.getLoadBalancerPool(lbid) >> pool
        _ * provider.getIpForInstance(id) >> ip
  ...
  def "should throw exception when failing to add member"() {
    ...
    LB_IDS.each { lbid ->
      INSTANCE_IDS.each { id ->
        ...
        provider.addMemberToLoadBalancerPool(ip, lbid, port, description.weight) >> { throw new OpenstackProviderException("foobar") }
      }
    }
    Exception ex = thrown(OpenstackProviderException)
    ex.message == "foobar"
  ...
  def "should throw exception when member id cannot be found for server instance"() {
    ...
    LB_IDS.each { lbid ->
      INSTANCE_IDS.each { id ->
        _ * provider.getLoadBalancerPool(lbid) >> pool
        _ * provider.getIpForInstance(id) >> ip
        provider.getMemberIdForInstance(ip, pool) >> { throw new OpenstackProviderException("foobar") }
      }
    }
    Exception ex = thrown(OpenstackProviderException)
    ex.message == "foobar"
  ...
  def "should throw exception when failing to remove member"() {
    ...
    LB_IDS.each { lbid ->
      INSTANCE_IDS.each { id ->
        _ * provider.getLoadBalancerPool(lbid) >> pool
        _ * provider.getIpForInstance(id) >> ip
        _ * provider.getMemberIdForInstance(ip, pool) >> memberId
        provider.removeMemberFromLoadBalancerPool(memberId) >> { throw new OpenstackProviderException("foobar") }
      }
    }
    Exception ex = thrown(OpenstackProviderException)
    ex.message == "foobar"
