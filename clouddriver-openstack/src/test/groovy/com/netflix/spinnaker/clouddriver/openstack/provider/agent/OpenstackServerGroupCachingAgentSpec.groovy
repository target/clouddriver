...
class OpenstackServerGroupCachingAgentSpec extends Specification {
  ...
  void "test build server group"() {
    ...
    Map<String, Object> asg = Mock(Map)
    ...
    1 * cachingAgent.buildASG(stack) >> asg
  ...
  void "test build image config"() {
    ...
    String imagekey = Keys.getImageKey(image, account)
  ...
  void "test build asg config - #testCase"() {
    when:
    Map<String, Object> result = cachingAgent.buildASG(stack)
    ...
    'empty'   | null                          | [:]
    'normal'  | buildStack(currentDate, 1, 5) | [createdTime: currentDate.time, minSize: 1, maxSize: 5]
    'missing' | buildStack(currentDate)       | [createdTime: currentDate.time, minSize: 0, maxSize: 0]
  }
  ...
  protected Stack buildStack(Date createDate, Integer minSize = null, Integer maxSize = null) {
    Stub(Stack) {
      getCreationTime() >> DateTimeFormatter.ISO_OFFSET_DATE_TIME.withZone(ZoneId.systemDefault()).format(createDate.toInstant())
